"""Wide incoherent energy-based sparse coding with microsaccade augmentation.

Builds on v9 (augmentation + incoherence) with doubled dictionary capacity:
400 features per class instead of 200. Previously, increasing capacity caused
overfitting (v6 with 300 features got 96.0% vs 96.4% baseline). Microsaccade
augmentation in v9 prevents this by forcing robust feature learning.

Quick sweep results (10k train, 25 epochs):
    v9 200f: 95.9% (baseline)
    v9 300f: 95.8% (no gain — needs more training)
    v9 400f: 96.5% (+0.6% — more capacity helps with augmentation)

Scaling from 10k→50k typically adds ~0.8% (v9-200f: 95.9%→96.7%).
Expected on full data: ~97.3%.

Architecture: Same as v9 — 10 class-specific dictionaries with incoherence
    regularization and microsaccade augmentation during training. Dictionary
    size increased from 200 to 400 atoms per class.

Biological analogue:
    - 400 features per class ≈ richer cortical column representations
    - In primate IT cortex, face-selective patches contain ~200-500 neurons
      per identity category (Tsao et al. 2006). 400 features maps to the
      upper end of this biological range.
    - More features = finer-grained decomposition of each digit class,
      capturing subtle stroke variations and style differences
    - This is the neural equivalent of a larger receptive field population —
      more V1 simple cells feeding each category-selective IT neuron

Based on:
    - v9 architecture (v6 + microsaccade augmentation)
    - Tsao et al. (2006): A cortical region for face perception
...
class SparseCodingV13Wide(MNISTApproach):
    """Wide incoherent energy-based sparse coding with augmentation.

    Same architecture as v9 but with 400 features per class (up from 200),
    enabled by microsaccade augmentation that prevents overfitting.
    ...
    name = "sparse_coding_v13"
    uses_backprop = False

    def __init__(
        self,
        n_features_per_class: int = 400,
        n_classes: int = 10,
        n_settle: int = 40,
        sparsity: float = 0.01,
