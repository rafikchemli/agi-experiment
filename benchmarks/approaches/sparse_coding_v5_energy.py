"""Energy-based sparse coding — class-specific dictionaries for classification.

Instead of learning one shared dictionary + readout, learn 10 separate
dictionaries (one per digit class). Each dictionary specializes in
reconstructing its own digit. Classification = which dictionary explains
the input best (lowest reconstruction error).

This directly aligns feature learning with classification — no readout
layer needed, no mismatch between reconstruction and discrimination.
...
class SparseCodingV5Energy(MNISTApproach):
